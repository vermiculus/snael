...
Now find occurances and store them in the dictionary prepared.  For
each name, go through every sentence to see if the name is in the
sentence.  If it is, add the occurance to the list in the
dictionary.
#+BEGIN_SRC python :tangle "./src/snael.py"
  for name in occurances.keys():
      for sentence in text:
          if name in sentence:
              occurances[name].append(text.index(sentence))
#+END_SRC

This is another meaty part of the program.  We declare a range =r=
(=range= is already a defined function in Python and cannot be used
as a variable name).  For each name, we search for other names within
the range =r= of this one.  So, if =r=5=, we will search for any
names that are within =r=5= sentences of each other.  For any pairs
of names that fall within range, we will add an association.
#+BEGIN_SRC python :tangle "./src/snael.py"
  print '>determining co-occurances'
  # determine co-occurances (range:5)
  network = Network()
  r = 5
  for name in occurances.keys():
      for oc in occurances[name]:
          for other in occurances.keys():
              for other_oc in occurances[other]:
                  if other_oc in range(oc-r, oc+r):
                      network.addConnection(name, other)
#+END_SRC
* TODO Output
** DONE Requirements
...
#+BEGIN_SRC python :tangle "./src/simhash.py"
# from http://blog.simpliplant.eu/calculating-similarity-between-text-strings-in-python/
import hashlib
...
class Simhash():
    ...
    def similarity(self, other_hash):
        ...
        if hash1 > hash2:
            return hash2/hash1*100
        else:
            return hash1/hash2*100
#+END_SRC
